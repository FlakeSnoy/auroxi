...
// Extends the auth 'user' table with Auroxi-specific fields
...
export const note = sqliteTable('note', {
  ...
  fileType:    text('file_type').notNull().default('text'), // text | image | pdf
...
export const message = sqliteTable('message', {
  id:         text('id').primaryKey().$defaultFn(() => crypto.randomUUID()),
  senderId:   text('sender_id').notNull(),
  receiverId: text('receiver_id'),   // null if group message
  groupId:    text('group_id'),      // null if DM
...
export const notification = sqliteTable('notification', {
  id:        text('id').primaryKey().$defaultFn(() => crypto.randomUUID()),
  userId:    text('user_id').notNull(),
  type:      text('type').notNull(), // friend_request | message | badge | system | lunes
  content:   text('content').notNull(),
...
export const lunesTransaction = sqliteTable('lunes_transaction', {
  ...
  type:      text('type').notNull(), // daily_claim | purchase | premium_spend
  createdAt: timestamp('created_at'),
});
...
export const report = sqliteTable('report', {
  ...
  targetType: text('target_type').notNull(), // message | note | user | group
